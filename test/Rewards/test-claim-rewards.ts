                        ...
                        describe("When there is one user claiming", async () => {
                            ...
                            describe("When the same user is trying to claim two sigs with nonce 0", async () => {
                                it("Should revert", async () => {
                                    ...
                                    await expect(protocol.yieldDistributor.connect(signers.random).claimRewards(sig2, ethers.constants.AddressZero, rewardee, amount)).to.be.revertedWith("signer must have permission from admin server role");
