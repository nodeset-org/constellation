...
describe("Liquidity Reserve", async function () {
    describe("base case scenario", async function () {
        it("should pass", async function () {
            ...
            await protocol.vCRPL.connect(signers.admin).setLiquidityReserveRatio(ethers.BigNumber.from("100000000000000000"));
            await protocol.vCWETH.connect(signers.admin).setLiquidityReserveRatio(ethers.BigNumber.from("100000000000000000"));
            ...
            await protocol.vCWETH.connect(signers.admin).setLiquidityReserveRatio(ethers.BigNumber.from("10000000000000000"));
            // Set the xRPL liquidity reserve to 5%
            await protocol.vCRPL.connect(signers.admin).setLiquidityReserveRatio(ethers.BigNumber.from("50000000000000000"));
