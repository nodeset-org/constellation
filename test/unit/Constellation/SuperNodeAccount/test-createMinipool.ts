...
describe('SuperNodeAccount.createMinipool', function () {
  ...
  beforeEach(async function () {
    ...
    const MockRocketDaoProtocolSettingsMinipool = await ethers.getContractFactory(
      'MockRocketDaoProtocolSettingsMinipool'
    );
    ...
    mockDepositData = {
      depositData: {
        pubkey: Buffer.from(
          '8c5e9f4d3b5a7e9d2f6b7c4e8f3d9e1f6c7e4d5b6a3f1c9d8e4f3b6c7a9d5e2b6c4e8f3a9d2e7c1f5d3b7e8c9d6a5',
          'hex'
        ),
      ...
      depositDataRoot: ethers.utils.arrayify('0x0fe98b4ef4b9a2a7cde5a8d9f1c6b7a8e4d3c2b1e8f4d2c3b6a9e7f5b3d8e6f9'),
  ...
  describe('when the protocol does not have enough liquidity', function () {
    it('should revert', async function () {
      await expect(
        superNodeAccount.connect(subNodeOperator).createMinipool(
          {
  ...
  describe('when the protocol has enough liquidity', function () {
    ...
    describe('when the sub node account is whitelisted', function () {
      ...
      describe('when the sub node operator is above the active minipool count', function () {
        it('should revert', async function () {
          await mockWhitelist.setActiveValidatorCountForOperator(subNodeOperator.address, 2);
          await expect(
            superNodeAccount.connect(subNodeOperator).createMinipool(
              {
                validatorPubkey: config.validatorPubkey,
