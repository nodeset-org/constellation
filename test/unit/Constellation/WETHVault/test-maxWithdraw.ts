...
describe("WETHVault.maxWithdraw", function () {
    ...
    describe("when the user has shares", function () {
        describe("when owner balance is greater than available liquidity", function () {
            // Note: This test does NOT test the accuracy/correctness of the maxWithdraw value
            ...
            it("returns the owner balance", async function () {
                ...
                const maxWithdraw = await wethVault.maxWithdraw(owner.address);
                expect(maxWithdraw).to.equal(ethers.utils.parseEther("1"));
            });
        });
