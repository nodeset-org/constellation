...
import { deployDev, deployDevUsingEnv } from '../../scripts/environments/deploy-dev';
import { getWalletFromPath } from '../../scripts/environments/keyReader';
import { deployToNetworkUsingEnv } from '../../scripts/environments/deploy-to-network';
...
describe(`Test Deploy Dev Env`, () => {
  ...
  it('Should pass', async () => {
    const directory = await deployToNetworkUsingEnv();
    ...
    const expectedProtocolSigner = await getWalletFromPath(
      ethers,
    ...
    expect(
      await directory?.hasRole(
        adminRole,
        (await getWalletFromPath(ethers, process.env.TEMPORAL_ADMIN_KEY_PATH as string)).address
      )
    ).equals(false);
    ...
    const nodesetRewards = await ethers.getContractAt(
      'NodeSetOperatorRewardDistributor',
      nodesetRewardsAddress as string
    );
    expect(await nodesetRewards?.hasRole(nodeSetServerAdminRole, process.env.NODESET_SERVER_ADMIN as string)).equals(
      true
    );
