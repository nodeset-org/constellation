    ...
    describe('when minting from below the coverage ratio', async function () {
      ...
      describe('when minting hits the bottom threshold (minWethRplRatio)', async function () {
        it('should pass', async function () {
          ...
          // Check if it reverts
          await expect(
            protocol.vCWETH.connect(signers.ethWhale).deposit(ethers.utils.parseEther('100'), signers.ethWhale.address)
          ).to.not.be.reverted;
        });
      });
      ...
      describe('when minting hits the top threshold (maxWethRplRatio)', async function () {
        it('should revert', async function () {
          ...
          // Check if it passes
          await expect(
            protocol.vCWETH.connect(signers.ethWhale).deposit(ethers.utils.parseEther('2000'), signers.ethWhale.address)
      ...
      describe('when minting hits the top threshold (maxWethRplRatio)', async function () {
        it('should pass', async function () {
          ...
          await protocol.wETH
            ...
            .approve(protocol.vCWETH.address, ethers.utils.parseEther('100'));
          await expect(
            protocol.vCWETH.connect(signers.ethWhale).deposit(ethers.utils.parseEther('100'), signers.ethWhale.address)
          ).to.not.be.reverted;
        });
      ...
      describe('when minting stays below the coverage ratio', async function () {
        it('should revert', async function () {
          ...
          await rocketPool.rplContract
            .connect(signers.ethWhale)
            .approve(protocol.vCRPL.address, ethers.utils.parseEther('10000'));

          await expect(
            protocol.vCRPL.connect(signers.ethWhale).deposit(ethers.utils.parseEther('10000'), signers.ethWhale.address)
    ...
    describe('when minting from bottom threshold (minWethRplRatio) of coverage ratio', async function () {
      describe('when minting goes below the coverage ratio', async function () {
        it('should revert', async function () {
          ...
          // Check that it reverts
          await expect(
            protocol.vCRPL.connect(signers.ethWhale).deposit(ethers.utils.parseEther('1'), signers.ethWhale.address)
          ).to.be.revertedWith('insufficient weth coverage ratio');
        });
      });
    });
      ...
      describe('when minting stays within ratio', async function () {
        it('should pass', async function () {
          ...
          await rocketPool.rplContract
            ...
            .transfer(signers.ethWhale.address, ethers.utils.parseEther('10000'));
          await rocketPool.rplContract
            .connect(signers.ethWhale)
            .approve(protocol.vCRPL.address, ethers.utils.parseEther('10000'));
          await expect(
