...
import {
  devParameterization,
  fastDeployProtocol,
  fastParameterization,
  generateBytes32Identifier,
  retryOperation,
...
export async function deployToNetworkUsingEnv(envName: string) {
  const dotenvPath = findConfig(`.env.${envName}`);
  ...
  if (
    !process.env.DEPLOYER_PRIVATE_KEY_PATH ||
    !process.env.DIRECTORY_DEPLOYER_PRIVATE_KEY_PATH
  ) {
