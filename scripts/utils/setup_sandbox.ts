...
export const setupSandbox = async (): Promise<SandboxDeployments> => {

    // TODO: ask teddy if okay to switch this? await ethers.getSigners();
    ...
    console.log("sanctions address", sanctions.address);
     ...
     const protocol = await fastDeployProtocol(deployer, admin, rocketStorage.address, wETH.address, sanctions.address, uniswapV3Pool.address, admin.address, true);
