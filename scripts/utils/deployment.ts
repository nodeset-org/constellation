...
export async function deployProtocol(signers: Signers, log = false): Promise<Protocol> {
    ...
    const timelockMediumRole = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("TIMELOCK_MED"));
    await directory.connect(signers.admin).grantRole(ethers.utils.arrayify(timelockMediumRole), signers.admin.address);
