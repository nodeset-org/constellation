...
export async function fastDeployProtocol(
  ...
  nodesetAdminAddress: string,
  nodesetServerAdminAddress: string,
  ...
  const yieldDistributorProxy = await retryOperation(async function () {
    const yd = await upgrades.deployProxy(
      ...
      [nodesetAdminAddress, nodesetServerAdminAddress],
      { initializer: 'initialize', kind: 'uups', unsafeAllow: ['constructor', 'delegatecall'] }
    );
  ...
  const treasuryProxy = await retryOperation(async function () {
    const at = await upgrades.deployProxy(await ethers.getContractFactory('Treasury', deployer), [treasurerAddress], {
...
export async function deployProtocol(signers: Signers, log = false): Promise<Protocol> {
  ...
  // set adminServer to be ADMIN_SERVER_ROLE
  const adminRole = ethers.utils.keccak256(ethers.utils.toUtf8Bytes('ADMIN_SERVER_ROLE'));
  let tx = await directory
    .connect(signers.admin)
    .grantRole(ethers.utils.arrayify(adminRole), signers.adminServer.address);
  await tx.wait();
  ...
  tx = await directory
    .connect(signers.admin)
    .grantRole(ethers.utils.arrayify(protocolRole), signers.protocolSigner.address);
  await tx.wait();
