...
contract YaspRPL is Base, Ownable, ERC20Burnable, ERC20Permit, ERC20FlashMint {
    ...
    function getRedemptionValuePerToken() public view returns (uint) {
        uint tvlRpl = DepositPool(getDirectory().getDepositPoolAddress()).getTvlRpl();
        assert(tvlRpl > 0);
    ...
    function getMinimumStakeError() public view returns (string memory) {
        return string.concat(string.concat("Minimum stake is ", Strings.toString(_minimumStakeAmount)), " RPL");
    }
