...
contract RocketDAOProtocolSettingsRewardsOld is
    ...
    RocketDAOProtocolSettingsRewardsInterfaceOld
{
    ...
    function setSettingRewardsClaimer(
        string memory _contractName,
        uint256 _perc
    ) public override onlyDAOProtocolProposal {
        ...
        setUint(
            keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount.updated.time', _contractName)),
            block.timestamp
        );
    }
    ...
    function getRewardsClaimerPercTimeUpdated(string memory _contractName) external view override returns (uint256) {
        return
            getUint(
                keccak256(
                    abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount.updated.time', _contractName)
                )
            );
    }

    // Get the perc amount total for all claimers (remaining goes to DAO)
    function getRewardsClaimersPercTotal() public view override returns (uint256) {
