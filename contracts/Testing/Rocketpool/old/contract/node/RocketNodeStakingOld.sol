...
contract RocketNodeStakingOld is RocketBase, RocketNodeStakingInterfaceOld {
    ...
    function getNodeETHMatched(address _nodeAddress) public view override returns (uint256) {
        ...
        if (ethMatched > 0) {
            ...
            RocketMinipoolManagerInterfaceOld rocketMinipoolManager = RocketMinipoolManagerInterfaceOld(
    ...
    function getNodeEffectiveRPLStake(address _nodeAddress) external view override returns (uint256) {
        // Load contracts
    ...
    function getNodeMinimumRPLStake(address _nodeAddress) external view override returns (uint256) {
        ...
        uint256 minimumStakePercent = rocketDAOProtocolSettingsNode.getMinimumPerMinipoolStake();
        uint256 matchedETH = getNodeETHMatched(_nodeAddress);
        return matchedETH.mul(minimumStakePercent).div(rocketNetworkPrices.getRPLPrice());
    }
    ...
    function getNodeMaximumRPLStake(address _nodeAddress) public view override returns (uint256) {
        ...
        uint256 maximumStakePercent = rocketDAOProtocolSettingsNode.getMaximumPerMinipoolStake();
        uint256 providedETH = getNodeETHProvided(_nodeAddress);
        return providedETH.mul(maximumStakePercent).div(rocketNetworkPrices.getRPLPrice());
    }
    ...
    function getNodeETHMatchedLimit(address _nodeAddress) external view override returns (uint256) {
        ...
        uint256 minimumStakePercent = rocketDAOProtocolSettingsNode.getMinimumPerMinipoolStake();
        return getNodeRPLStake(_nodeAddress).mul(rocketNetworkPrices.getRPLPrice()).div(minimumStakePercent);
    }
    ...
    function stakeRPL(
        uint256 _amount
    ) external override onlyLatestContract('rocketNodeStaking', address(this)) onlyRegisteredNode(msg.sender) {
    ...
    function stakeRPLFor(
        ...
        uint256 _amount
    ) external override onlyLatestContract('rocketNodeStaking', address(this)) onlyRegisteredNode(_nodeAddress) {
    ...
    function withdrawRPL(
        ...
        require(
            block.timestamp.sub(getNodeRPLStakedTime(msg.sender)) >=
        ...
        require(
            rplStake.sub(_amount) >= getNodeMaximumRPLStake(msg.sender),
        ...
        rocketVault.withdrawToken(
            rocketStorage.getNodeWithdrawalAddress(msg.sender),
