...
contract RocketDepositPool is RocketBase, RocketDepositPoolInterface, RocketVaultWithdrawerInterface {
    ...
    function getNodeBalance() public view override returns (uint256) {
    ...
    function getUserBalance() public view override returns (int256) {
        ...
        */
        ...
        if (capacityNeeded > maxDepositPoolSize) {
            ...
            if (rocketDAOProtocolSettingsDeposit.getAssignDepositsEnabled()) {
                ...
                require(
                    capacityNeeded <= maxDepositPoolSize.add(rocketMinipoolQueue.getEffectiveCapacity()),
    ...
    ) external payable override onlyThisLatestContract onlyLatestContract('rocketNodeDeposit', msg.sender) {
    ...
    function nodeCreditWithdrawal(
        uint256 _amount
    ) external override onlyThisLatestContract onlyLatestContract('rocketNodeDeposit', msg.sender) {
    ...
    function _assignDeposits(
        RocketDAOProtocolSettingsDepositInterface _rocketDAOProtocolSettingsDeposit
    ...
    function _assignDepositsNew(
        RocketMinipoolQueueInterface _rocketMinipoolQueue,
        RocketDAOProtocolSettingsDepositInterface _rocketDAOProtocolSettingsDeposit
    ) private returns (bool) {
        // Load contracts
        ...
        uint256 assignments = _rocketDAOProtocolSettingsDeposit.getMaximumDepositSocialisedAssignments().add(
            scalingCount
        );
    ...
    function _assignDepositsLegacy(
        RocketMinipoolQueueInterface _rocketMinipoolQueue,
        RocketDAOProtocolSettingsDepositInterface _rocketDAOProtocolSettingsDeposit
    ) private returns (bool) {
        // Load contracts
    ...
    function withdrawExcessBalance(
        uint256 _amount
    ) external override onlyThisLatestContract onlyLatestContract('rocketTokenRETH', msg.sender) {
        // Check amount
