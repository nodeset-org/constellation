...
contract RocketClaimDAO is RocketBase, RocketClaimDAOInterface {
    ...
    uint256 internal constant existsOffset = 0;
    uint256 internal constant recipientOffset = 1;
    uint256 internal constant amountOffset = 2;
    uint256 internal constant periodLengthOffset = 3;
    uint256 internal constant lastPaymentOffset = 4;
    uint256 internal constant numPeriodsOffset = 5;
    uint256 internal constant periodsPaidOffset = 6;

    // Events
    event RPLTokensSentByDAOProtocol(
    ...
    event RPLTreasuryContractPayment(
    ...
    event RPLTreasuryContractCreated(
        string indexed contractName,
        address indexed recipient,
    ...
    event RPLTreasuryContractUpdated(
        string indexed contractName,
        address indexed recipient,
        uint256 amountPerPeriod,
        uint256 periodLength,
    ...
    function getContract(string calldata _contractName) external view override returns (PaymentContract memory) {
        // Compute key
        uint256 contractKey = uint256(keccak256(abi.encodePacked('dao.protocol.treasury.contract', _contractName)));
        // Retrieve details
    ...
    function getBalance(address _recipientAddress) external view override returns (uint256) {
    ...
    function spend(
        ...
        external
        override
        onlyLatestContract('rocketDAOProtocolProposals', msg.sender)
        onlyLatestContract('rocketClaimDAO', address(this))
    {
        // Load contracts
        ...
        // Some initial checks
        require(
            _amount > 0 && _amount <= rocketVault.balanceOfToken('rocketClaimDAO', rplToken),
        ...
        onlyLatestContract('rocketClaimDAO', address(this))
    {
        ...
        // Log it
        emit RPLTreasuryContractCreated(
            _contractName,
            _recipientAddress,
            _amountPerPeriod,
            _startTime,
    ...
    function withdrawBalance(
        address _recipientAddress
    ) external override onlyLatestContract('rocketClaimDAO', address(this)) {
        // Load contracts
        RocketVaultInterface rocketVault = RocketVaultInterface(getContractAddress('rocketVault'));
        // Addresses
        IERC20 rplToken = IERC20(getContractAddress('rocketTokenRPL'));
        ...
        require(
            amount <= rocketVault.balanceOfToken('rocketClaimDAO', rplToken),
    ...
    ) external onlyLatestContract('rocketClaimDAO', address(this)) {
