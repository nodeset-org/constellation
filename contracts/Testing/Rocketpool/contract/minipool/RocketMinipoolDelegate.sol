...
contract RocketMinipoolDelegate is RocketMinipoolStorageLayout, RocketMinipoolInterface {
    ...
    function getPreLaunchValue() external view override returns (uint256) {
        return preLaunchValue;
    }
    function getNodeTopUpValue() external view override returns (uint256) {
        return nodeDepositBalance.sub(preLaunchValue);
    }
    ...
    function getPreMigrationBalance() external view override returns (uint256) {
        return preMigrationBalance;
    }
    function getUserDistributed() external view override returns (bool) {
        return userDistributed;
    }

    // User deposit detail getters
    function getUserDepositBalance() public view override returns (uint256) {
    ...
    function initialise(address _nodeAddress) external override onlyUninitialised {
        // Check parameters
    ...
    function deposit() external payable override onlyLatestContract('rocketDepositPool', msg.sender) onlyInitialised {
        // Check current status & node deposit status
    ...
    function canPromote() external view override onlyInitialised returns (bool) {
        // Check status
        if (status != MinipoolStatus.Prelaunch) {
            return false;
        }
    ...
    ) external override onlyLatestContract('rocketMinipoolManager', msg.sender) onlyInitialised {
        // Check status
    ...
    function promote() external override onlyMinipoolOwner(msg.sender) onlyInitialised {
        // Check status
        ...
        RocketNodeDepositInterface rocketNodeDepositInterface = RocketNodeDepositInterface(
    ...
    function beginUserDistribute() external override onlyInitialised {
        require(status == MinipoolStatus.Staking, 'Minipool must be staking');
        uint256 totalBalance = address(this).balance.sub(nodeRefundBalance);
        ...
        require(
            rocketDAOProtocolSettingsMinipool.hasUserDistributeWindowPassed(timeElapsed),
    ...
    function userDistributeAllowed() public view override returns (bool) {
        // Get contracts
        RocketDAOProtocolSettingsMinipoolInterface rocketDAOProtocolSettingsMinipool = RocketDAOProtocolSettingsMinipoolInterface(
                getContractAddress('rocketDAOProtocolSettingsMinipool')
            );
    ...
    function reduceBondAmount() external override onlyMinipoolOwner(msg.sender) onlyInitialised {
        require(status == MinipoolStatus.Staking, 'Minipool must be staking');
        ...
        RocketMinipoolBondReducerInterface rocketBondReducer = RocketMinipoolBondReducerInterface(
            getContractAddress('rocketMinipoolBondReducer')
        );
