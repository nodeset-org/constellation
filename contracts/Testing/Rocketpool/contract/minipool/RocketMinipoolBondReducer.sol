...
contract RocketMinipoolBondReducer is RocketBase, RocketMinipoolBondReducerInterface {
    ...
    function beginReduceBondAmount(
    ...
    ) external override onlyLatestContract('rocketMinipoolBondReducer', address(this)) {
        ...
        RocketDAOProtocolSettingsMinipoolInterface daoSettingsMinipool = RocketDAOProtocolSettingsMinipoolInterface(
        ...
        bool reductionCancelled = getBool(
            keccak256(abi.encodePacked('minipool.bond.reduction.cancelled', address(minipool)))
        );
        ...
        uint256 lastReduction = getUint(
            keccak256(abi.encodePacked('minipool.last.bond.reduction.time', _minipoolAddress))
        );
        uint256 rewardInterval = daoSettingsRewards.getRewardsClaimIntervalTime();
        require(
            block.timestamp >= lastReduction.add(rewardInterval),
            'Not enough time has passed since last bond reduction'
        );
    ...
    function getReduceBondTime(address _minipoolAddress) external view override returns (uint256) {
    ...
    /// @param _minipoolAddress Address of the minipool to query
    function getReduceBondValue(address _minipoolAddress) external view override returns (uint256) {
    ...
    /// @param _minipoolAddress Address of the minipool to query
    function getReduceBondCancelled(address _minipoolAddress) public view override returns (bool) {
    ...
    /// @param _minipoolAddress Address of the minipool
    function canReduceBondAmount(address _minipoolAddress) public view override returns (bool) {
    ...
    /// @param _minipoolAddress Address of the minipool
    function voteCancelReduction(
        address _minipoolAddress
    ) external override onlyTrustedNode(msg.sender) onlyLatestContract('rocketMinipoolBondReducer', address(this)) {
        ...
        bytes32 memberVotedKey = keccak256(
            abi.encodePacked('minipool.bond.reduction.member.voted', _minipoolAddress, msg.sender)
        );
        ...
        uint256 quorum = rocketDAONode
            .getMemberCount()
            .mul(rocketDAONodeTrustedSettingsMinipool.getCancelBondReductionQuorum())
            .div(calcBase);
        bytes32 totalCancelVotesKey = keccak256(
            abi.encodePacked('minipool.bond.reduction.vote.count', _minipoolAddress)
        );
    ...
    function reduceBondAmount()
        ...
        onlyLatestContract('rocketMinipoolBondReducer', address(this))
        returns (uint256)
    {
        // Get contracts
        RocketNodeDepositInterface rocketNodeDeposit = RocketNodeDepositInterface(
        ...
        bool reductionCancelled = getBool(
            keccak256(abi.encodePacked('minipool.bond.reduction.cancelled', address(msg.sender)))
        );
        require(!reductionCancelled, 'This minipool is not allowed to reduce bond');
    ...
    function getLastBondReductionTime(address _minipoolAddress) external view override returns (uint256) {
    ...
    function getLastBondReductionPrevValue(address _minipoolAddress) external view override returns (uint256) {
    ...
    function getLastBondReductionPrevNodeFee(address _minipoolAddress) external view override returns (uint256) {
