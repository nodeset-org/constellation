...
contract RocketDAOSecurityProposals is RocketBase, RocketDAOSecurityProposalsInterface {
    ...
    string internal constant daoNameSpace = 'dao.security.';

    // The namespace of the DAO that setting changes get applied to (protocol DAO)
    string internal constant protocolDaoSettingNamespace = 'dao.protocol.setting.';
    ...
    modifier onlySecurityMember() {
        require(
            getBool(keccak256(abi.encodePacked(daoNameSpace, 'member', msg.sender))),
            'Sender is not a security council member'
        );
        _;
    }
    ...
    function propose(
        ...
        override
        onlySecurityMember
        onlyLatestContract('rocketDAOSecurityProposals', address(this))
        returns (uint256)
    {
        // Load contracts
        ...
        RocketDAOProtocolSettingsSecurityInterface rocketDAOProtocolSettingsSecurity = RocketDAOProtocolSettingsSecurityInterface(
                getContractAddress('rocketDAOProtocolSettingsSecurity')
            );
        // Create the proposal
        return
            daoProposal.add(
                ...
                _proposalMessage,
                block.timestamp + 1,
                rocketDAOProtocolSettingsSecurity.getVoteTime(),
                rocketDAOProtocolSettingsSecurity.getExecuteTime(),
                daoSecurity.getMemberQuorumVotesRequired(),
                _payload
            );
    }
    ...
    function vote(
        uint256 _proposalID,
        bool _support
    ) external override onlySecurityMember onlyLatestContract('rocketDAOSecurityProposals', address(this)) {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress('rocketDAOProposal'));
        RocketDAOSecurityInterface daoSecurity = RocketDAOSecurityInterface(getContractAddress('rocketDAOSecurity'));
        // Did they join after this proposal was created? If so, they can't vote or it'll throw off the set proposalVotesRequired
        require(
            daoSecurity.getMemberJoinedTime(msg.sender) < daoProposal.getCreated(_proposalID),
    ...
    function execute(
        uint256 _proposalID
    ) external override onlyLatestContract('rocketDAOSecurityProposals', address(this)) {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress('rocketDAOProposal'));
        // Execute now
    ...
    function proposalSettingUint(
        ...
        uint256 _value
    ) public override onlyExecutingContracts onlyValidSetting(_settingNameSpace, _settingPath) {
    ...
    ) public override onlyLatestContract('rocketDAOProtocolProposals', msg.sender) {
        // Their proposal executed, record the block
        setUint(
            keccak256(abi.encodePacked(daoNameSpace, 'member.executed.time', 'invited', _memberAddress)),
            block.timestamp
        );
    ...
    function proposalKick(
        ...
        // Load contracts
        RocketDAOSecurityActionsInterface daoActionsContract = RocketDAOSecurityActionsInterface(
            getContractAddress('rocketDAOSecurityActions')
        );
        // Kick them now
    ...
    function proposalReplace(
        address _existingMemberAddress,
        string calldata _newMemberId,
        address _newMemberAddress
    ) external override onlyLatestContract('rocketDAOProtocolProposals', msg.sender) {
