...
contract RocketDAOSecurityActions is RocketBase, RocketDAOSecurityActionsInterface {
    ...
    ) public override onlyLatestContract('rocketDAOSecurityProposals', msg.sender) {
    ...
    function actionKickMulti(
        address[] calldata _memberAddresses
    ) external override onlyLatestContract('rocketDAOSecurityProposals', msg.sender) {
        // Remove the members
    ...
    function actionJoin() external override onlyLatestContract('rocketDAOSecurityActions', address(this)) {
        // Add the member
    ...
    function actionRequestLeave() external override onlyLatestContract('rocketDAOSecurityActions', address(this)) {
        // Load contracts
        RocketDAOSecurityInterface rocketDAOSecurity = RocketDAOSecurityInterface(
            getContractAddress('rocketDAOSecurity')
        );
        ...
        setUint(
            keccak256(abi.encodePacked(daoNameSpace, 'member.executed.time', 'leave', msg.sender)),
            block.timestamp + rocketDAOProtocolSettingsSecurity.getLeaveTime()
        );
        // Log it
    ...
    function actionLeave() external override onlyLatestContract('rocketDAOSecurityActions', address(this)) {
        // Load contracts
        RocketDAOSecurityInterface rocketDAOSecurity = RocketDAOSecurityInterface(
            getContractAddress('rocketDAOSecurity')
        );
        ...
        require(
            leaveAcceptedTime + rocketDAOProtocolSettingsSecurity.getActionTime() > block.timestamp,
    ...
    function _memberJoin(address _nodeAddress) private {
        ...
        require(
            memberInvitedTime + rocketDAOProtocolSettingsSecurity.getActionTime() > block.timestamp,
            "This node's invitation to join has expired, please apply again"
        );
        // Check current node status
        require(
            rocketDAOSecurity.getMemberIsValid(_nodeAddress) != true,
            'This node is already part of the security council'
        );
