...
contract RocketDAOProposal is RocketBase, RocketDAOProposalInterface {
    ...
    string private constant daoProposalNameSpace = 'dao.proposal.';

    // Only allow the DAO contract to access
    modifier onlyDAOContract(string memory _daoName) {
        // Load contracts
        require(
            keccak256(abi.encodePacked(getContractName(msg.sender))) == keccak256(abi.encodePacked(_daoName)),
    ...
    function getDAO(uint256 _proposalID) public view override returns (string memory) {
    ...
    function getEnd(uint256 _proposalID) public view override returns (uint256) {
    ...
    function getVotesFor(uint256 _proposalID) public view override returns (uint256) {
    ...
    function getVotesAgainst(uint256 _proposalID) public view override returns (uint256) {
    ...
    function getVotesRequired(uint256 _proposalID) public view override returns (uint256) {
    ...
    function getCancelled(uint256 _proposalID) public view override returns (bool) {
    ...
    function getReceiptHasVoted(uint256 _proposalID, address _nodeAddress) public view override returns (bool) {
        return
            getBool(keccak256(abi.encodePacked(daoProposalNameSpace, 'receipt.hasVoted', _proposalID, _nodeAddress)));
    }

    // Returns true if this proposal was supported by this member
    function getReceiptSupported(uint256 _proposalID, address _nodeAddress) external view override returns (bool) {
        return
            getBool(keccak256(abi.encodePacked(daoProposalNameSpace, 'receipt.supported', _proposalID, _nodeAddress)));
    }
    ...
    ) external override onlyDAOContract(_dao) returns (uint256) {
        // Basic checks
    ...
    function vote(
        ...
        uint256 _proposalID,
        bool _support
    ) external override onlyDAOContract(getDAO(_proposalID)) {
    ...
    function cancel(address _member, uint256 _proposalID) external override onlyDAOContract(getDAO(_proposalID)) {
        // Firstly make sure this proposal can be cancelled
        require(
            getState(_proposalID) == ProposalState.Pending || getState(_proposalID) == ProposalState.Active,
