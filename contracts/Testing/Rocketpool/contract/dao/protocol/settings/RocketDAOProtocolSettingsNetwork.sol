...
contract RocketDAOProtocolSettingsNetwork is RocketDAOProtocolSettings, RocketDAOProtocolSettingsNetworkInterface {
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) public override onlyDAOProtocolProposal {
        ...
        if (settingKey == keccak256(bytes('network.consensus.threshold'))) {
            ...
            require(
                _value >= 0.05 ether && _value <= 0.2 ether,
                'The node fee minimum must be a value between 5% and 20%'
            );
        } else if (settingKey == keccak256(bytes('network.node.fee.target'))) {
            require(
                _value >= 0.05 ether && _value <= 0.2 ether,
                'The node fee target must be a value between 5% and 20%'
            );
        } else if (settingKey == keccak256(bytes('network.node.fee.maximum'))) {
            require(
                _value >= 0.05 ether && _value <= 0.2 ether,
                'The node fee maximum must be a value between 5% and 20%'
            );
