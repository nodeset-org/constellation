...
contract RocketDAOProtocolSettingsInflation is RocketDAOProtocolSettings, RocketDAOProtocolSettingsInflationInterface {
    ...
    ) RocketDAOProtocolSettings(_rocketStorageAddress, 'inflation') {
        version = 2;
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) public override onlyDAOProtocolProposal {
        ...
        if (getBool(keccak256(abi.encodePacked(settingNameSpace, 'deployed')))) {
            ...
            if (settingKey == keccak256(bytes('rpl.inflation.interval.start'))) {
                ...
                require(
                    _value <= getSettingUint('rpl.inflation.interval.rate') + 0.01 ether,
                    'No greater than 1e16 more than the previous value'
                );
