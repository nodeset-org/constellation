...
contract RocketDAOProtocolSettingsRewards is RocketDAOProtocolSettings, RocketDAOProtocolSettingsRewardsInterface {
    constructor(
    ...
    ) RocketDAOProtocolSettings(_rocketStorageAddress, 'rewards') {
        version = 2;
        ...
        if (!getBool(keccak256(abi.encodePacked(settingNameSpace, 'deployed')))) {
            ...
            setUint(
                keccak256(
                    abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimTrustedNode')
                ),
                0.2 ether
            );
            setUint(
                keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimDAO')),
                0.1 ether
            );
            setUint(
                keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimNode')),
                0.7 ether
            );
            setUint(
                keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount.updated.time')),
                block.timestamp
            );
    ...
    ) external override onlyDAOProtocolProposal {
        // Check total
        require(_trustedNodePercent + _protocolPercent + _nodePercent == 1 ether, 'Total does not equal 100%');
        // Update now
        setUint(
            keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimTrustedNode')),
            _trustedNodePercent
        );
        setUint(
            keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimDAO')),
    ...
    function getRewardsClaimerPerc(string memory _contractName) public view override returns (uint256) {
    ...
    function getRewardsClaimersPerc()
        ...
        view
        override
        returns (uint256 trustedNodePerc, uint256 protocolPerc, uint256 nodePerc)
    {
        trustedNodePerc = getUint(
            keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimTrustedNode'))
        );
        protocolPerc = getUint(
            keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimDAO'))
        );
        nodePerc = getUint(
            keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimNode'))
    ...
    function getRewardsClaimersTrustedNodePerc() public view override returns (uint256) {
        return
            getUint(
                keccak256(
                    abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimTrustedNode')
    ...
    function getRewardsClaimersProtocolPerc() public view override returns (uint256) {
        return
            getUint(keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimDAO')));
    }

    /// @notice Get the percentage of rewards paid to the node operators
    function getRewardsClaimersNodePerc() public view override returns (uint256) {
        return
            getUint(keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'group.amount', 'rocketClaimNode')));
    }

    /// @notice Get the time of when the claim percentages were last updated
    function getRewardsClaimersTimeUpdated() external view override returns (uint256) {
    ...
    function getRewardsClaimIntervalPeriods() external view override returns (uint256) {
    ...
    function getRewardsClaimIntervalTime() external view override returns (uint256) {
        ...
        return
            getUint(keccak256(abi.encodePacked(settingNameSpace, 'rewards.claims', 'periods'))) *
            rocketDAOProtocolSettingsNetwork.getSubmitBalancesFrequency();
    }
}
