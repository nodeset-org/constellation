...
contract RocketDAONodeTrustedActions is RocketBase, RocketDAONodeTrustedActionsInterface {
    ...
    event ActionChallengeMade(
        address indexed nodeChallengedAddress,
        address indexed nodeChallengerAddress,
        uint256 time
    );
    event ActionChallengeDecided(
        address indexed nodeChallengedAddress,
    ...
    string private constant daoNameSpace = 'dao.trustednodes.';
    ...
    function _memberAdd(address _nodeAddress, uint256 _rplBondAmountPaid) private {
        ...
        require(
            rocketDAONode.getMemberIsValid(_nodeAddress) != true,
        ...
        if (_rplBondAmountPaid > 0)
            setUint(keccak256(abi.encodePacked(daoNameSpace, 'member.bond.rpl', _nodeAddress)), _rplBondAmountPaid);
    ...
    function _memberJoin(address _nodeAddress) private {
        ...
        RocketDAONodeTrustedSettingsProposalsInterface rocketDAONodeTrustedSettingsProposals = RocketDAONodeTrustedSettingsProposalsInterface(
                getContractAddress('rocketDAONodeTrustedSettingsProposals')
            );
        ...
        require(
            memberInvitedTime.add(rocketDAONodeTrustedSettingsProposals.getActionTime()) > block.timestamp,
        ...
        require(
            rplInflationContract.allowance(_nodeAddress, address(this)) >= rplBondAmount,
        ...
        require(
            rplInflationContract.transferFrom(_nodeAddress, address(this), rplBondAmount),
        ...
        require(
            rplInflationContract.approve(rocketVaultAddress, rplBondAmount),
    ...
    function actionJoin()
        ...
        onlyLatestContract('rocketDAONodeTrustedActions', address(this))
    {
    ...
    function actionJoinRequired(
        address _nodeAddress
    ) external override onlyRegisteredNode(_nodeAddress) onlyLatestContract('rocketDAONodeTrusted', msg.sender) {
    ...
    function actionLeave(
        address _rplBondRefundAddress
    ) external override onlyTrustedNode(msg.sender) onlyLatestContract('rocketDAONodeTrustedActions', address(this)) {
        // Load contracts
        ...
        require(
            rocketDAONode.getMemberCount() > rocketDAONode.getMemberMinRequired(),
        ...
        require(
            leaveAcceptedTime.add(rocketDAONodeTrustedSettingsProposals.getActionTime()) > block.timestamp,
    ...
    function actionKick(
        address _nodeAddress,
        uint256 _rplFine
    ) external override onlyTrustedNode(_nodeAddress) onlyLatestContract('rocketDAONodeTrustedProposals', msg.sender) {
        // Load contracts
        RocketVaultInterface rocketVault = RocketVaultInterface(getContractAddress('rocketVault'));
        RocketDAONodeTrustedInterface rocketDAONode = RocketDAONodeTrustedInterface(
            getContractAddress('rocketDAONodeTrusted')
        ...
        if (rplBondRefundAmount > 0) {
            ...
            if (rplToken.balanceOf(address(rocketVault)) >= rplBondRefundAmount)
                rocketVault.withdrawToken(
                    _nodeAddress,
                    IERC20(getContractAddress('rocketTokenRPL')),
                    rplBondRefundAmount
    ...
    function actionChallengeMake(
        address _nodeAddress
    )
        external
        payable
        override
        onlyTrustedNode(_nodeAddress)
        onlyRegisteredNode(msg.sender)
        onlyLatestContract('rocketDAONodeTrustedActions', address(this))
    {
        // Load contracts
        ...
        if (rocketDAONode.getMemberIsValid(msg.sender) != true)
            require(
                msg.value == rocketDAONodeTrustedSettingsMembers.getChallengeCost(),
        ...
        require(
            getUint(keccak256(abi.encodePacked(daoNameSpace, 'node.challenge.created.time', msg.sender))).add(
    ...
    function actionChallengeDecide(
        address _nodeAddress
    )
        external
        override
        ...
        onlyLatestContract('rocketDAONodeTrustedActions', address(this))
    {
        ...
        if (_nodeAddress == msg.sender) {
            ...
            require(
                challengeTime.add(rocketDAONodeTrustedSettingsMembers.getChallengeWindow()) < block.timestamp,
