...
contract RocketDAONodeTrustedSettingsRewards is
    ...
    RocketDAONodeTrustedSettingsRewardsInterface
{
    ...
    function setSettingBool(string memory _settingPath, bool _value) public override onlyDAONodeTrustedProposal {
        // Some safety guards for certain settings
        if (getBool(keccak256(abi.encodePacked(settingNameSpace, 'deployed')))) {
            // oDAO should never disable main net rewards
            if (
                keccak256(abi.encodePacked(_settingPath)) ==
                keccak256(abi.encodePacked('rewards.network.enabled', uint256(0)))
            ) {
    ...
    function getNetworkEnabled(uint256 _network) external view override returns (bool) {
