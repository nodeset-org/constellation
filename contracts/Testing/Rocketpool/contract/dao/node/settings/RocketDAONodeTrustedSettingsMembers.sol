...
contract RocketDAONodeTrustedSettingsMembers is
    ...
    RocketDAONodeTrustedSettingsMembersInterface
{
    ...
    ) RocketDAONodeTrustedSettings(_rocketStorageAddress, 'members') {
        // Set version
        version = 1;
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) public override onlyDAONodeTrustedProposal {
        ...
        if (keccak256(abi.encodePacked(_settingPath)) == keccak256(abi.encodePacked('members.quorum')))
            require(_value > 0 ether && _value <= 0.9 ether, 'Quorum setting must be > 0 & <= 90%');
        // Update setting now
    ...
    function getRPLBond() external view override returns (uint256) {
    ...
    function getMinipoolUnbondedMax() external view override returns (uint256) {
    ...
    function getMinipoolUnbondedMinFee() external view override returns (uint256) {
    ...
    function getChallengeCooldown() external view override returns (uint256) {
    ...
    function getChallengeCost() external view override returns (uint256) {
