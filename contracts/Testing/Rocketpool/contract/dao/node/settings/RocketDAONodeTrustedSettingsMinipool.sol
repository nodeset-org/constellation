...
contract RocketDAONodeTrustedSettingsMinipool is
    ...
    RocketDAONodeTrustedSettingsMinipoolInterface
{
    ...
    ) RocketDAONodeTrustedSettings(_rocketStorageAddress, 'minipool') {
        // Set version
        version = 2;
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) public override onlyDAONodeTrustedProposal {
        ...
        if (getBool(keccak256(abi.encodePacked(settingNameSpace, 'deployed')))) {
            if (keccak256(abi.encodePacked(_settingPath)) == keccak256(abi.encodePacked('minipool.scrub.period'))) {
                ...
                require(
                    _value <= (rocketDAOProtocolSettingsMinipool.getLaunchTimeout().sub(1 hours)),
    ...
    function getScrubPeriod() external view override returns (uint256) {
    ...
    function getPromotionScrubPeriod() external view override returns (uint256) {
    ...
    function getScrubQuorum() external view override returns (uint256) {
    ...
    function getCancelBondReductionQuorum() external view override returns (uint256) {
    ...
    function getScrubPenaltyEnabled() external view override returns (bool) {
    ...
    function isWithinBondReductionWindow(uint256 _time) external view override returns (bool) {
    ...
    function getBondReductionWindowStart() public view override returns (uint256) {
        return getSettingUint('minipool.bond.reduction.window.start');
    }

    /// @notice Returns the length of the bond reduction window
    function getBondReductionWindowLength() public view override returns (uint256) {
