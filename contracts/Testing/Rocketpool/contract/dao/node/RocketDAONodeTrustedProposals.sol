...
contract RocketDAONodeTrustedProposals is RocketBase, RocketDAONodeTrustedProposalsInterface {
    ...
    modifier onlyExecutingContracts() {
        ...
        require(
            msg.sender == getContractAddress('rocketDAONodeTrusted') ||
    ...
    function propose(
        ...
        onlyLatestContract('rocketDAONodeTrustedProposals', address(this))
        returns (uint256)
    {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress('rocketDAOProposal'));
        RocketDAONodeTrustedInterface daoNodeTrusted = RocketDAONodeTrustedInterface(
        ...
        require(
            daoNodeTrusted.getMemberLastProposalTime(msg.sender).add(
                rocketDAONodeTrustedSettingsProposals.getCooldownTime()
            ) <= block.timestamp,
        ...
        return
            daoProposal.add(
                ...
                _proposalMessage,
                block.timestamp.add(rocketDAONodeTrustedSettingsProposals.getVoteDelayTime()),
                rocketDAONodeTrustedSettingsProposals.getVoteTime(),
                rocketDAONodeTrustedSettingsProposals.getExecuteTime(),
                daoNodeTrusted.getMemberQuorumVotesRequired(),
                _payload
            );
    }
    ...
    function vote(
        uint256 _proposalID,
        bool _support
    ) external override onlyTrustedNode(msg.sender) onlyLatestContract('rocketDAONodeTrustedProposals', address(this)) {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress('rocketDAOProposal'));
        RocketDAONodeTrustedInterface daoNodeTrusted = RocketDAONodeTrustedInterface(
            getContractAddress('rocketDAONodeTrusted')
    ...
    function execute(
        uint256 _proposalID
    ) external override onlyLatestContract('rocketDAONodeTrustedProposals', address(this)) {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress('rocketDAOProposal'));
        // Execute now
    ...
    function proposalInvite(
        string memory _id,
        string memory _url,
        address _nodeAddress
    ) external override onlyExecutingContracts onlyRegisteredNode(_nodeAddress) {
        // Their proposal executed, record the block
        setUint(
            keccak256(abi.encodePacked(daoNameSpace, 'member.executed.time', 'invited', _nodeAddress)),
            block.timestamp
        );
    ...
    function proposalLeave(
        address _nodeAddress
    ) external override onlyExecutingContracts onlyTrustedNode(_nodeAddress) {
        // Load contracts
        RocketDAONodeTrustedInterface daoNodeTrusted = RocketDAONodeTrustedInterface(
            getContractAddress('rocketDAONodeTrusted')
        );
        ...
        setUint(
            keccak256(abi.encodePacked(daoNameSpace, 'member.executed.time', 'leave', _nodeAddress)),
            block.timestamp
        );
    }

    // Propose to kick a current member from the DAO with an optional RPL bond fine
    function proposalKick(
        address _nodeAddress,
        uint256 _rplFine
    ) external override onlyExecutingContracts onlyTrustedNode(_nodeAddress) {
        // Load contracts
        ...
        RocketDAONodeTrustedActionsInterface daoActionsContract = RocketDAONodeTrustedActionsInterface(
            getContractAddress('rocketDAONodeTrustedActions')
        );
        ...
        require(
            _rplFine <= rplBondAmount,
            'RPL Fine must be lower or equal to the RPL bond amount of the node being kicked'
        );
        // Set their bond amount minus the fine
        setUint(
            keccak256(abi.encodePacked(daoNameSpace, 'member.bond.rpl', _nodeAddress)),
            rplBondAmount.sub(_rplFine)
        );
        // Kick them now
    ...
    function proposalSettingUint(
        ...
        // Load contracts
        RocketDAONodeTrustedSettingsInterface rocketDAONodeTrustedSettings = RocketDAONodeTrustedSettingsInterface(
    ...
    function proposalUpgrade(
        ...
        // Load contracts
        RocketDAONodeTrustedUpgradeInterface rocketDAONodeTrustedUpgradeInterface = RocketDAONodeTrustedUpgradeInterface(
                getContractAddress('rocketDAONodeTrustedUpgrade')
            );
        // Lets update
    ...
    function _memberInit(
        ...
        string memory _url,
        address _nodeAddress
    ) private onlyRegisteredNode(_nodeAddress) {
        // Load contracts
        RocketDAONodeTrustedInterface daoNodeTrusted = RocketDAONodeTrustedInterface(
            getContractAddress('rocketDAONodeTrusted')
        );
