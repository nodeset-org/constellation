...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    function getEthValue(uint256 _rethAmount) public view override returns (uint256) {
        ...
        RocketNetworkBalancesInterface rocketNetworkBalances = RocketNetworkBalancesInterface(
            getContractAddress('rocketNetworkBalances')
        );
    ...
    function mint(
        uint256 _ethAmount,
        address _to
    ) external override onlyLatestContract('rocketDepositPool', msg.sender) {
        // Get rETH amount
