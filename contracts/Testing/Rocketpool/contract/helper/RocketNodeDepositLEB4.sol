...
contract RocketNodeDepositLEB4 is RocketBase, RocketNodeDepositInterface {
    ...
    function getNodeDepositCredit(address _nodeOperator) public view override returns (uint256) {
    ...
    function getNodeEthBalance(address _nodeAddress) public view override returns (uint256) {
    ...
    function getNodeCreditAndBalance(address _nodeAddress) external view override returns (uint256) {
    ...
    function getNodeUsableCreditAndBalance(address _nodeAddress) external view override returns (uint256) {
    ...
    function getNodeUsableCredit(address _nodeAddress) public view override returns (uint256) {
    ...
    function increaseDepositCreditBalance(
        address _nodeOperator,
        uint256 _amount
    ) external override onlyLatestContract('rocketNodeDeposit', address(this)) {
    ...
    function depositEthFor(address _nodeAddress) external payable override onlyRegisteredMinipool(_nodeAddress) {
    ...
    function isValidDepositAmount(uint256 _amount) public pure override returns (bool) {
        return _amount == 16 ether || _amount == 8 ether || _amount == 4 ether;
    }

    /// @notice Returns an array of valid deposit amounts
    function getDepositAmounts() external pure override returns (uint256[] memory) {
    ...
    function _deposit(
        ...
        if (msg.value < preLaunchValue) {
            ...
            require(
                preLaunchValue - msg.value <= rocketDepositPool.getBalance(),
                'Deposit pool balance is insufficient for pre deposit'
            );
        }
        ...
        minipool.preDeposit{value: preLaunchValue}(
            _bondAmount,
            _validatorPubkey,
            _validatorSignature,
            _depositDataRoot
    ...
    ) external override onlyLatestContract('rocketNodeDeposit', address(this)) onlyRegisteredNode(msg.sender) {
        // Check pre-conditions
    ...
    function increaseEthMatched(
        address _nodeAddress,
        uint256 _amount
    ) external override onlyLatestContract('rocketNodeDeposit', address(this)) onlyLatestNetworkContract {
    ...
    /// @param _expectedMinipoolAddress The expected minipool address. Reverts if not correct
    /// @param _currentBalance The current balance of the validator on the beaconchain (will be checked by oDAO and scrubbed if not correct)
    function _createVacantMinipool(
        uint256 _salt,
        bytes calldata _validatorPubkey,
        uint256 _bondAmount,
        address _expectedMinipoolAddress,
        uint256 _currentBalance
    ) private returns (RocketMinipoolInterface) {
        // Load contracts
        RocketMinipoolManagerInterface rocketMinipoolManager = RocketMinipoolManagerInterface(
            getContractAddress('rocketMinipoolManager')
        );
        ...
        // Create minipool
        RocketMinipoolInterface minipool = rocketMinipoolManager.createVacantMinipool(
            msg.sender,
            _salt,
            _validatorPubkey,
            _bondAmount,
