...
contract RocketAuctionManager is RocketBase, RocketAuctionManagerInterface {
    ...
    function getTotalRPLBalance() public view override returns (uint256) {
    ...
    function getAllottedRPLBalance() public view override returns (uint256) {
    ...
    function getRemainingRPLBalance() public view override returns (uint256) {
        return getTotalRPLBalance().sub(getAllottedRPLBalance());
    }

    // Get/set the number of lots for auction
    function getLotCount() public view override returns (uint256) {
    ...
    // Get lot details
    function getLotExists(uint256 _index) public view override returns (bool) {
        return getBool(keccak256(abi.encodePacked('auction.lot.exists', _index)));
    }
    function getLotStartBlock(uint256 _index) public view override returns (uint256) {
        return getUint(keccak256(abi.encodePacked('auction.lot.block.start', _index)));
    }
    function getLotEndBlock(uint256 _index) public view override returns (uint256) {
        return getUint(keccak256(abi.encodePacked('auction.lot.block.end', _index)));
    }
    function getLotStartPrice(uint256 _index) public view override returns (uint256) {
        return getUint(keccak256(abi.encodePacked('auction.lot.price.start', _index)));
    }
    function getLotReservePrice(uint256 _index) public view override returns (uint256) {
        return getUint(keccak256(abi.encodePacked('auction.lot.price.reserve', _index)));
    }
    function getLotTotalRPLAmount(uint256 _index) public view override returns (uint256) {
        return getUint(keccak256(abi.encodePacked('auction.lot.rpl.total', _index)));
    }

    // Get/set the total ETH amount bid on a lot
    function getLotTotalBidAmount(uint256 _index) public view override returns (uint256) {
    ...
    function getLotAddressBidAmount(uint256 _index, address _bidder) public view override returns (uint256) {
    ...
    function getLotRPLRecovered(uint256 _index) public view override returns (bool) {
    ...
    function getLotPriceAtBlock(uint256 _index, uint256 _block) public view override returns (uint256) {
        // Get lot parameters
    ...
    function getLotPriceAtCurrentBlock(uint256 _index) public view override returns (uint256) {
        return getLotPriceAtBlock(_index, block.number);
    }

    // Get the RPL price for a lot based on total ETH amount bid
    function getLotPriceByTotalBids(uint256 _index) public view override returns (uint256) {
    ...
    function getLotCurrentPrice(uint256 _index) public view override returns (uint256) {
        uint256 blockPrice = getLotPriceAtCurrentBlock(_index);
        uint256 bidPrice = getLotPriceByTotalBids(_index);
        if (bidPrice > blockPrice) {
            return bidPrice;
        } else {
            return blockPrice;
        }
    ...
    function getLotClaimedRPLAmount(uint256 _index) public view override returns (uint256) {
    ...
    function getLotRemainingRPLAmount(uint256 _index) public view override returns (uint256) {
        return getLotTotalRPLAmount(_index).sub(getLotClaimedRPLAmount(_index));
    }

    // Check whether a lot has cleared
    function getLotIsCleared(uint256 _index) external view override returns (bool) {
        if (block.number >= getLotEndBlock(_index)) {
            return true;
        }
        if (getLotPriceByTotalBids(_index) >= getLotPriceAtCurrentBlock(_index)) {
            return true;
        }
        return false;
    }
    ...
    function createLot() external override onlyLatestContract('rocketAuctionManager', address(this)) {
        // Load contracts
        RocketDAOProtocolSettingsAuctionInterface rocketAuctionSettings = RocketDAOProtocolSettingsAuctionInterface(
            getContractAddress('rocketDAOProtocolSettingsAuction')
        );
        RocketNetworkPricesInterface rocketNetworkPrices = RocketNetworkPricesInterface(
            getContractAddress('rocketNetworkPrices')
        );
        ...
        require(
            remainingRplBalance >= calcBase.mul(rocketAuctionSettings.getLotMinimumEthValue()).div(rplPrice),
        ...
        if (lotRplAmount > maximumLotRplAmount) {
            lotRplAmount = maximumLotRplAmount;
        }
        // Create lot
        ...
        setUint(
            keccak256(abi.encodePacked('auction.lot.block.end', lotIndex)),
            block.number.add(rocketAuctionSettings.getLotDuration())
        );
        setUint(
            keccak256(abi.encodePacked('auction.lot.price.start', lotIndex)),
            rplPrice.mul(rocketAuctionSettings.getStartingPriceRatio()).div(calcBase)
        );
        setUint(
            keccak256(abi.encodePacked('auction.lot.price.reserve', lotIndex)),
            rplPrice.mul(rocketAuctionSettings.getReservePriceRatio()).div(calcBase)
        );
    ...
    function placeBid(
        uint256 _lotIndex
    ) external payable override onlyLatestContract('rocketAuctionManager', address(this)) {
        // Load contracts
        RocketDAOProtocolSettingsAuctionInterface rocketAuctionSettings = RocketDAOProtocolSettingsAuctionInterface(
            getContractAddress('rocketDAOProtocolSettingsAuction')
        );
        ...
        if (msg.value > bidAmount) {
            msg.sender.transfer(msg.value.sub(bidAmount));
        }
    ...
    function claimBid(uint256 _lotIndex) external override onlyLatestContract('rocketAuctionManager', address(this)) {
        // Check lot exists
        require(getLotExists(_lotIndex), 'Lot does not exist');
        // Get lot price info
    ...
    function recoverUnclaimedRPL(
        uint256 _lotIndex
    ) external override onlyLatestContract('rocketAuctionManager', address(this)) {
