...
contract RocketNodeManager is RocketBase, RocketNodeManagerInterface {
    ...
    function getNodeCount() public view override returns (uint256) {
    ...
    function getNodeCountPerTimezone(
        uint256 _offset,
        uint256 _limit
    ) external view override returns (TimezoneCount[] memory) {
        // Get contracts
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(
            getContractAddress('addressSetStorage')
        );
    ...
    function getNodeAt(uint256 _index) external view override returns (address) {
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(
            getContractAddress('addressSetStorage')
        );
    ...
    function getNodeExists(address _nodeAddress) public view override returns (bool) {
    ...
    function getNodePendingWithdrawalAddress(address _nodeAddress) public view override returns (address) {
        return rocketStorage.getNodePendingWithdrawalAddress(_nodeAddress);
    }

    /// @notice Get a node's current RPL withdrawal address
    function getNodeRPLWithdrawalAddress(address _nodeAddress) public view override returns (address) {
        address withdrawalAddress = getAddress(
            keccak256(abi.encodePacked('node.rpl.withdrawal.address', _nodeAddress))
        );
    ...
    function getNodePendingRPLWithdrawalAddress(address _nodeAddress) public view override returns (address) {
    ...
    function getNodeRPLWithdrawalAddressIsSet(address _nodeAddress) external view override returns (bool) {
    ...
    function setRPLWithdrawalAddress(
        ...
        bool _confirm
    ) external override onlyRegisteredNode(_nodeAddress) {
        ...
        // Set pending withdrawal address if not confirmed
            ...
            setAddress(
                keccak256(abi.encodePacked('node.pending.rpl.withdrawal.address', _nodeAddress)),
                _newRPLWithdrawalAddress
            );
        }
    }
    ...
    function getNodeTimezoneLocation(address _nodeAddress) public view override returns (string memory) {
    ...
    function registerNode(
    ...
    ) external override onlyLatestContract('rocketNodeManager', address(this)) {
        // Load contracts
        ...
        setUint(
            keccak256(abi.encodePacked('rewards.pool.claim.contract.registered.time', 'rocketClaimNode', msg.sender)),
            block.timestamp
        );
        // Update count
        rocketNetworkSnapshots.push(
            keccak256(abi.encodePacked('node.count')),
            uint32(block.number),
            uint224(addressSetStorage.getCount(nodeIndexKey))
        );
        // Default voting delegate to themself
        rocketNetworkSnapshots.push(
            keccak256(abi.encodePacked('node.delegate', msg.sender)),
            uint32(block.number),
            uint224(uint160(msg.sender))
        );
    ...
    function getNodeRegistrationTime(
        address _nodeAddress
    ) public view override onlyRegisteredNode(_nodeAddress) returns (uint256) {
        return
            getUint(
                keccak256(
                    abi.encodePacked('rewards.pool.claim.contract.registered.time', 'rocketClaimNode', _nodeAddress)
                )
            );
    }
    ...
    function setTimezoneLocation(
        string calldata _timezoneLocation
    ) external override onlyLatestContract('rocketNodeManager', address(this)) onlyRegisteredNode(msg.sender) {
        // Check timezone location
        require(bytes(_timezoneLocation).length >= 4, 'The timezone location is invalid');
    ...
    function getFeeDistributorInitialised(address _nodeAddress) public view override returns (bool) {
        // Load contracts
        RocketNodeDistributorFactoryInterface rocketNodeDistributorFactory = RocketNodeDistributorFactoryInterface(
            getContractAddress('rocketNodeDistributorFactory')
        );
    ...
    function initialiseFeeDistributor()
        external
        override
        onlyLatestContract('rocketNodeManager', address(this))
        ...
        if (count > 0) {
            ...
            for (uint256 i = 0; i < count; i++) {
                RocketMinipoolInterface minipool = RocketMinipoolInterface(
                    rocketMinipoolManager.getNodeMinipoolAt(msg.sender, i)
                );
    ...
    function getAverageNodeFee(address _nodeAddress) external view override returns (uint256) {
        // Load contracts
        RocketMinipoolManagerInterface rocketMinipoolManager = RocketMinipoolManagerInterface(
            getContractAddress('rocketMinipoolManager')
        );
        RocketNodeDepositInterface rocketNodeDeposit = RocketNodeDepositInterface(
            getContractAddress('rocketNodeDeposit')
        );
        ...
        for (uint256 i = 0; i < depositSizes.length; i++) {
            if (depositCounts[i] > 0) {
                ...
                if (depositSizes[i] == 16 ether) {
                    ...
                    numeratorKey = keccak256(
                        abi.encodePacked('node.average.fee.numerator', _nodeAddress, depositSizes[i])
                    );
                }
    ...
    function setRewardNetwork(
        ...
        require(
            ...
            "Only a tx from a node's withdrawal address can change reward network"
        );
        // Check network is enabled
        RocketDAONodeTrustedSettingsRewardsInterface rocketDAONodeTrustedSettingsRewards = RocketDAONodeTrustedSettingsRewardsInterface(
                getContractAddress('rocketDAONodeTrustedSettingsRewards')
            );
    ...
    function getRewardNetwork(
        address _nodeAddress
    ) public view override onlyLatestContract('rocketNodeManager', address(this)) returns (uint256) {
    ...
    function setSmoothingPoolRegistrationState(
        ...
        RocketDAOProtocolSettingsNodeInterface daoSettingsNode = RocketDAOProtocolSettingsNodeInterface(
            getContractAddress('rocketDAOProtocolSettingsNode')
        );
    ...
    function getSmoothingPoolRegistrationChanged(address _nodeAddress) external view override returns (uint256) {
    ...
    function getSmoothingPoolRegisteredNodeCount(
        uint256 _offset,
        uint256 _limit
    ...
    function getNodeDetails(address _nodeAddress) public view override returns (NodeDetails memory nodeDetails) {
        // Get contracts
    ...
    function getNodeAddresses(uint256 _offset, uint256 _limit) external view override returns (address[] memory) {
        // Get contracts
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(
            getContractAddress('addressSetStorage')
        );
