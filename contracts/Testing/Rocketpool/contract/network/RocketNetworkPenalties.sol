...
contract RocketNetworkPenalties is RocketBase, RocketNetworkPenaltiesInterface {
    ...
    function submitPenalty(
        ...
        external
        override
        onlyLatestContract('rocketNetworkPenalties', address(this))
        ...
        onlyRegisteredMinipool(_minipoolAddress)
    {
        ...
        bytes32 nodeSubmissionKey = keccak256(
            abi.encodePacked('network.penalties.submitted.node', msg.sender, _minipoolAddress, _block)
        );
        bytes32 submissionCountKey = keccak256(
            abi.encodePacked('network.penalties.submitted.count', _minipoolAddress, _block)
        );
        ...
        if (
            calcBase.mul(submissionCount).div(rocketDAONodeTrusted.getMemberCount()) >=
    ...
    function executeUpdatePenalty(
        address _minipoolAddress,
        uint256 _block
    ) external override onlyLatestContract('rocketNetworkPenalties', address(this)) {
        // Get contracts
        RocketDAOProtocolSettingsNetworkInterface rocketDAOProtocolSettingsNetwork = RocketDAOProtocolSettingsNetworkInterface(
                getContractAddress('rocketDAOProtocolSettingsNetwork')
            );
    ...
    function getPenaltyCount(address _minipoolAddress) external view override returns (uint256) {
    ...
    function incrementMinipoolPenaltyCount(address _minipoolAddress) private {
        ...
        RocketMinipoolPenaltyInterface rocketMinipoolPenalty = RocketMinipoolPenaltyInterface(
