...
contract RocketNetworkVoting is RocketBase, RocketNetworkVotingInterface {
    // Constants
    bytes32 internal immutable priceKey;
    ...
    function initialiseVoting() external override onlyRegisteredNode(msg.sender) {
        ...
        rocketNetworkSnapshots.push(
            key,
            uint32(block.number),
            uint224(rocketNodeStaking.getNodeETHMatched(msg.sender))
        );
        ...
        rocketNetworkSnapshots.push(
            key,
            uint32(block.number),
            uint224(rocketMinipoolManager.getNodeActiveMinipoolCount(msg.sender))
        );
    ...
    function getVotingInitialised(address _nodeAddress) external view override returns (bool) {
    ...
    function getNodeCount(uint32 _block) external view override returns (uint256) {
        // Get contracts
        RocketNetworkSnapshotsInterface rocketNetworkSnapshots = RocketNetworkSnapshotsInterface(
            getContractAddress('rocketNetworkSnapshots')
        );
    ...
    /// @param _block Block number to query
    function getVotingPower(address _nodeAddress, uint32 _block) external view override returns (uint256) {
    ...
    function getDelegate(address _nodeAddress, uint32 _block) external view override returns (address) {
        RocketNetworkSnapshotsInterface rocketNetworkSnapshots = RocketNetworkSnapshotsInterface(
            getContractAddress('rocketNetworkSnapshots')
        );
    ...
    function getCurrentDelegate(address _nodeAddress) external view override returns (address) {
        RocketNetworkSnapshotsInterface rocketNetworkSnapshots = RocketNetworkSnapshotsInterface(
            getContractAddress('rocketNetworkSnapshots')
        );
        bytes32 key = keccak256(abi.encodePacked('node.delegate', _nodeAddress));
